...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, target_actions=None):
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
        ...
        return action_preds[0,-1] + 0.01 * torch.randn_like(action_preds[0,-1])


class StochDecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.predict_action_log_std = nn.Linear(hidden_size, self.act_dim)
        self.min_std_log = -20
        self.max_std_log = 2
        self.predict_return = torch.nn.Linear(hidden_size, 1)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, target_actions=None):
        ...
        means = self.predict_action_mean(x[:,1])  # predict next action mean given state
        action_pred_log_std = self.predict_action_log_std(x[:,1])  # predict next action log std given state
        action_pred_log_std = torch.clamp(action_pred_log_std, self.min_std_log, self.max_std_log)
        stds = torch.exp(action_pred_log_std)
